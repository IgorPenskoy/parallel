...
int **copy_board(int **board, int width, int height) {
    int **board_copy = NULL;
    
    board_copy = (int **) calloc(height, sizeof(int *));
    memset(board_copy, 0, height * sizeof(int *));
    for (int i = 0; i < height; i++) {
        board_copy[i] = (int *) calloc(width, sizeof(int));
...
path_t *init_path(tour_info_t tour_info) {
    ...
    step_t *first_step = init_first_step(tour_info.x_pos, tour_info.y_pos);

    path->board[tour_info.y_pos][tour_info.x_pos] = 1;
...
int **find_path(tour_info_t tour_info) {
    int tour_exists = 0;
    ...
    while (!tour_exists && !tour_not_exist) {
        ...
        if (condition == TOUR_EXISTS) {
            board = copy_board(path->board, tour_info.width, tour_info.height);
            tour_exists = 1;
        }
